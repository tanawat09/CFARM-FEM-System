...
@section('page_title', 'ตรวจเช็ค' . ($equipment->type == 'emergency_light' ? 'ไฟฉุกเฉิน' : 'ที่ล้างตา/ฝักบัวฉุกเฉิน'))

@section('content')
<div class="card shadow-sm border-0 mb-4 rounded-4 overflow-hidden">
    <div class="bg-{{ $equipment->type == 'emergency_light' ? 'warning text-dark' : 'info text-white' }} p-3">
        <div class="d-flex align-items-center">
            <i class="bi bi-{{ $equipment->type == 'emergency_light' ? 'lightbulb' : 'droplet' }} display-6 me-3"></i>
            <div>
                <h5 class="mb-0 fw-bold">{{ $equipment->asset_code }}</h5>
                <small>{{ $equipment->type_name }}</small>
            </div>
        </div>
    </div>
    <div class="card-body p-3 bg-white">
        <div class="row text-sm">
            <div class="col-6 mb-2">
                ...
                <span class="fw-semibold text-dark">{{ $equipment->location->location_name ?? 'N/A' }}</span>
            </div>
            <div class="col-6 mb-2">
                <span class="text-muted d-block small">อาคาร/โซน:</span>
                <span class="fw-semibold text-dark">{{ $equipment->house ?? '-' }} / {{ $equipment->zone ?? '-' }}</span>
            </div>
            <div class="col-6 mb-2">
                <span class="text-muted d-block small">ยี่ห้อ:</span>
                <span class="fw-semibold">{{ $equipment->brand ?? '-' }}</span>
            </div>
            <div class="col-6 mb-2">
                <span class="text-muted d-block small">รุ่น:</span>
                <span class="fw-semibold">{{ $equipment->model ?? '-' }}</span>
            </div>
        </div>
    </div>
</div>

<form id="inspectionForm" action="{{ route('equipment-inspections.store') }}" method="POST">
    @csrf
