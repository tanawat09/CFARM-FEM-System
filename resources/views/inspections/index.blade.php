...
<div class="row mb-4 align-items-center">
    <div class="col-md-7 mb-3 mb-md-0">
        <form action="{{ route('inspections.index') }}" method="GET" class="d-flex gap-2 flex-wrap">
            <input
                type="month"
                name="month"
                value="{{ $selectedMonth }}"
            ...
            @if(request('location_id') || $selectedMonth !== now()->format('Y-m'))
                <a href="{{ route('inspections.index') }}" class="btn btn-light rounded-pill border shadow-sm" title="ล้างตัวกรอง">
    ...
    <div class="col-md-5 text-md-end">
        <a href="{{ route('inspections.create') }}" class="btn btn-primary rounded-pill fw-medium shadow-sm px-4">
            <i class="bi bi-shield-plus me-2"></i> บันทึกการตรวจเช็ค
        </a>
    </div>
</div>
...
<div class="row g-3 mb-4">
    <div class="col-md-3">
        <div class="card border-0 shadow-sm rounded-4 h-100">
            <div class="card-body">
                <small class="text-muted d-block mb-2">เดือนที่เลือก</small>
                <h5 class="fw-bold mb-1">{{ $selectedMonthLabel }}</h5>
                <div class="text-muted">สรุปข้อมูลการตรวจประจำเดือน</div>
            </div>
        </div>
    </div>
    <div class="col-md-3">
        <div class="card border-0 shadow-sm rounded-4 h-100">
            <div class="card-body">
                <small class="text-muted d-block mb-2">ตรวจทั้งหมด</small>
                <h2 class="fw-bold mb-0">{{ number_format($summary['total']) }}</h2>
            </div>
        </div>
    </div>
    <div class="col-md-3">
        <div class="card border-0 shadow-sm rounded-4 h-100">
            <div class="card-body">
                <small class="text-muted d-block mb-2">ผ่าน</small>
                <h2 class="fw-bold text-success mb-0">{{ number_format($summary['passed']) }}</h2>
            </div>
        </div>
    </div>
    <div class="col-md-3">
        <div class="card border-0 shadow-sm rounded-4 h-100">
            <div class="card-body">
                <small class="text-muted d-block mb-2">ไม่ผ่าน / แจ้งซ่อม</small>
                <h2 class="fw-bold text-danger mb-1">{{ number_format($summary['failed']) }}</h2>
                <div class="text-muted">อัตราผ่าน {{ number_format($summary['pass_rate'], 2) }}%</div>
            </div>
        </div>
    </div>
</div>
...
<div class="card border-0 shadow-sm rounded-4 overflow-hidden">
            ...
            <table class="table table-hover align-middle mb-0">
                <thead class="bg-primary text-white">
                    <tr>
                        ...
                        <th class="py-3 border-0 fw-medium">อุปกรณ์ / พื้นที่</th>
                        <th class="py-3 border-0 fw-medium">ผู้ตรวจ</th>
                        <th class="py-3 border-0 fw-medium">วันเวลา</th>
                ...
                <tbody>
                    @forelse($inspections as $ins)
                        <tr>
                            ...
                            <td>
                                @if($ins->fireExtinguisher)
                                    ...
                                    <small class="text-muted">
                                        <i class="bi bi-geo-alt me-1 text-danger"></i> {{ $ins->fireExtinguisher->location->location_name ?? 'ไม่ระบุ' }}
                                    </small>
                                @else
                                    ...
                                    <small class="text-muted">
                                        <i class="bi bi-geo-alt me-1 opacity-50"></i> ไม่ทราบพื้นที่
                                    </small>
                                @endif
                            </td>
                            <td>
                            ...
                            <td>
                                @php
                                    $inspectedAt = $ins->inspected_at->copy()->timezone(config('app.timezone'));
                                @endphp
                                <div class="text-dark fw-medium">
                                    {{ $inspectedAt->locale('th')->translatedFormat('d M') }} {{ $inspectedAt->year + 543 }}
                                </div>
                            ...
                            <td class="text-center">
                                ...
                                @elseif($ins->overall_result == 'fail')
                                    <span class="badge bg-danger bg-opacity-10 text-danger border border-danger border-opacity-50 px-3 py-2 rounded-pill">
                                        <i class="bi bi-x-circle-fill me-1"></i> ชำรุด/แจ้งซ่อม
                                    </span>
                                @else
                                    <span class="badge bg-warning bg-opacity-10 text-warning border border-warning border-opacity-50 px-3 py-2 rounded-pill">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-5 text-muted">
                                ...
                                <h5 class="fw-normal">ไม่พบข้อมูลการตรวจเช็คในเดือนที่เลือก</h5>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    @if(isset($inspections) && $inspections->hasPages())
        <div class="card-footer bg-white border-top border-light p-3 d-flex justify-content-center">
            {{ $inspections->appends(request()->query())->links('pagination::bootstrap-5') }}
        </div>
    @endif
</div>
@endsection
