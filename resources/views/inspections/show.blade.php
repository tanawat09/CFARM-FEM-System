    ...
    <div class="col-md-8">
        <div class="card border-0 shadow-sm rounded-4 mb-4">
            <div class="card-header bg-white border-bottom p-4 d-flex justify-content-between align-items-center">
                ...
                <a href="{{ route('inspections.index') }}" class="btn btn-light btn-sm rounded-pill px-3">
            ...
            <div class="card-body p-4 p-md-5">
                @php
                    $inspectedAt = $inspection->inspected_at->copy()->timezone(config('app.timezone'));
                @endphp
                ...
                <div class="row mb-5 bg-light p-4 rounded-4 align-items-center">
                    <div class="col-md-6 border-end-md">
                        ...
                        <span class="text-secondary ms-4 ps-2">
                            <i class="bi bi-geo-alt"></i>
                            {{ $inspection->fireExtinguisher->location->location_name ?? '-' }}
                            (ชั้น {{ $inspection->fireExtinguisher->location->floor ?? '-' }})
                        </span>
                    </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-12">
                                ...
                                <span class="fw-semibold text-dark">
                                    ...
                                    {{ $inspectedAt->locale('th')->translatedFormat('d F') }}
                                    {{ $inspectedAt->year + 543 }}
                                    เวลา {{ $inspectedAt->format('H:i') }} น.
                                </span>
                            </div>
                        </div>
                    </div>
            ...
            <div class="card-footer bg-light p-3 text-center d-print-none">
                <button onclick="window.print()" class="btn btn-secondary rounded-pill px-4">
                    <i class="bi bi-printer me-1"></i> พิมพ์รายงานนี้
                </button>
            </div>
        </div>
    </div>
